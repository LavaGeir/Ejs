...
app.get('/', async (req, res) => { // Definerer rute for å vise startsiden
  try { // Starter try/catch for feilhandtering
    const songs = await dbAll( // Henter alle sanger fra databasen
      'SELECT id, title, artist, nationality, listened_date FROM songs ORDER BY listened_date DESC, id DESC', // SQL for å liste sanger
...
app.post('/songs', async (req, res) => { // Definerer rute for innsending av nytt sangskjema
  try { // Starter try/catch for å håndtere feil
    const { title, artist, nationality, listened_date } = req.body; // Leser ut feltene fra skjemaet
    if (!title || !artist || !nationality || !listened_date) { // Sjekker at alle felt er utfylt
      const songs = await dbAll('SELECT id, title, artist, nationality, listened_date FROM songs ORDER BY listened_date DESC, id DESC'); // Henter liste for visning ved feil
      return res.status(400).render('index', { title: 'Registrer sanger', songs, message: 'Fyll ut tittel, artist, nasjonalitet og dato.' }); // Viser feilmelding
    ...
    await dbRun( // Kjører INSERT for å lagre sangen
      'INSERT INTO songs (title, artist, nationality, listened_date) VALUES (?, ?, ?, ?)', // SQL med parametere
      [title.trim(), artist.trim(), nationality.trim(), listened_date] // Verdier å sette inn (trim fjerner ekstra mellomrom)
    ); // Avslutter INSERT
